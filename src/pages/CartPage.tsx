...
export default function CartPage(){
    ...
    const getProducts = async () => {
        try {
            const productResponse = await axios.get("https://shoewizardsdb.azurewebsites.net/smartcart-cart/cart", {
                headers: {
                    ...
                    <div className="mx-20">
                        <div className="row-span-8 my-10 py-7 bg-white rounded-3xl">
                            ...
                            <div className="mx-9 justify-items-center">
                                <CartCard data={products} />
                            </div>
                            { products.length === 0 ? (
                                null
                            ) : (
                                <>
                                    <div className="flex py-10 mx-9 border-t border-shoewizards-light-blue justify-between items-center">
                                        <h3 className="text-shoewizards-light-blue text-2xl font-bold">Total Price</h3>
                                        <h3 className="text-shoewizards-light-blue text-2xl font-bold">
                                            Rp {" "}
                                            {products.reduce((acc, curr) => acc + curr.price * curr.quantity, 0).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".")},00
                                        </h3>
                                    </div>
                                    <div className="flex mx-9 justify-center">
                                        <button className="w-full text-shoewizards-light-blue bg-white border-2 border-shoewizards-light-blue shadow-xl hover:bg-shoewizards-light-blue hover:text-white font-bold text-md rounded-full px-5 py-2.5 text-center"
